...
//		}
		...
		gd.addChoice("Data for the brightness measure", titles, titles[1]); //Data
...
///////		
		...
		for(int ii=0; ii<sumpx; ii++){
			int pixval=ip3.get(ii);
			if(maxvalue<pixval)
			maxvalue=(double) pixval;
		}
...
/////////////////////Start: signal detection///////////////////////////////
		...
		for(int n=0; n<sumpx; n++){
			...
			if(pix1>200){//Mask value
				...
				if(pix3>0)
				value1 = value1.add(BigDecimal.valueOf(pix3));//total sum brightness
			}//	if(pix1>200){
		}//for(int n=0; n<sumpx; n++){
		...
		double mean3 = mean2.doubleValue();
		double maxvalue2=0;
	...
	//		IJ.log("maxvalue; "+maxvalue2+"	 gap;	"+gap);  for debug
			...
			for(int n3=0; n3<sumpx; n3++){
				...
				if(pix1>200){//Mask value
					...
					if(imp.getType()==imp.GRAY16 && maxvalue<4096){
						if(pix3>4095)
						pix3=4095;
					...
					value11 = value11.add(BigDecimal.valueOf(pix3));//total sum brightness
